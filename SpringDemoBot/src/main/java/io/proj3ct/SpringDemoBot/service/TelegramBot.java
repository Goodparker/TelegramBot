...
public class TelegramBot extends TelegramLongPollingBot{
            ...
            try {
                switch (messageText)
                {
                    ...
                    case "/joke":
                        ...
                        break;
                    case "/game":
                        cubeCommandReceived(chatId);
                        break;
                    case "/sovmes":
                        sovmesCommandReceived(chatId);
                        break;
    ...
    private void cubeCommandReceived(long chatId) throws IOException {

        String rez;
        int randomNumberBot = (int) (Math.random() * 6) + 1;
        int randomNumberUser = (int) (Math.random() * 6) + 1;
        if(randomNumberBot>randomNumberUser)
        {
             rez = "Ты бросил: " + randomNumberUser + "  Бот бросил: " + randomNumberBot + "  Бот выиграл";
        }
        else
        {
             rez = "Ты бросил: " + randomNumberUser + "  Бот бросил: " + randomNumberBot + "  Ты выиграл";
        }
        if (randomNumberBot==randomNumberUser) {
            rez = "Ты бросил: " + randomNumberUser + "  Бот бросил: " + randomNumberBot + "  Ничья";
        }

        sendMessage(chatId, rez);

    }
    private void sovmesCommandReceived(long chatId) throws IOException {
        String manName = null, womenName = null, firstMes = null;
        Update update = new Update();
        String usermes = update.getMessage().getText();
        if(firstMes == null)
        {
            firstMes = usermes;
        }else if(womenName == null)
        {
            womenName = usermes;
        ...
        Document doc = Jsoup.connect("https://my-calend.ru/sovmestimost-imen/womenName/manName").get();
        var titleEl =  doc.selectFirst("p");
        //System.out.println(titleEl.text());
        String rez = titleEl.text();
        sendMessage(chatId, rez);
